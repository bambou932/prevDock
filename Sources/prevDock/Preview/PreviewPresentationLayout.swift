...
enum PreviewCardChromeLayout {
    ...
    static func compactConstraints(
        container: NSView,
        appIconView: NSView,
        titleLabel: NSView,
        statusLabel: NSView,
        ...
        let trailingInset: CGFloat = closeButton == nil ? 12 : 42
        var constraints = [
            appIconView.leadingAnchor.constraint(equalTo: container.leadingAnchor, constant: 10),
            appIconView.centerYAnchor.constraint(equalTo: container.centerYAnchor),
            ...
            statusLabel.widthAnchor.constraint(lessThanOrEqualTo: container.widthAnchor, multiplier: 0.30)
        ]
        if let closeButton {
...
enum PreviewPresentationLayout {
    ...
    static func scrollPosition(in stack: NSStackView) -> NSPoint? {
        stack.arrangedSubviews.compactMap { $0 as? NSScrollView }.first?.contentView.bounds.origin
    }

    static func restoreScrollPosition(_ position: NSPoint?, in stack: NSStackView) {
        guard let position,
              let scrollView = stack.arrangedSubviews.compactMap({ $0 as? NSScrollView }).first,
        ...
        clipView.scroll(to: NSPoint(
            x: min(maximumX, max(0, position.x)),
            y: min(maximumY, max(0, position.y))
        ))
    ...
    private static func makeScrollView(
        ...
        let scrollView = PreviewPresentationScrollView()
...
private final class PreviewPresentationScrollView: NSScrollView {
    override func scrollWheel(with event: NSEvent) {
        guard hasHorizontalScroller, !hasVerticalScroller,
        ...
        let position = NSPoint(
            x: min(maximumX, max(0, contentView.bounds.minX - delta)),
