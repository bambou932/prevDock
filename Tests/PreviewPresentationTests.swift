...
private enum PreviewPresentationTests {
    static func main() {
        let tests: [TestCase] = [
            ("one-row auto-fit installs every planned card once without scrolling", testAutoPresentation),
            ("grouped one-row auto-fit installs every card without scrolling", testGroupedAutoPresentation),
            ("one-row auto-fit rejects mismatched card inventory", testAutoInventoryMismatch),
            ...
            ("compact list chrome leaves title and close button reachable", testCompactChromeGeometry),
            ("compact list scrolling retains position and clamps after removal", testCompactScrollPosition),
            ("ordinary mouse wheel moves horizontal overflow", testHorizontalWheel)
        ]
    ...
    private static func testGroupedAutoPresentation() throws {
        ...
        let input = PreviewLayoutInput(
            ...
            availablePanelSize: NSSize(width: 800, height: 600),
            backingScale: 2
        )
        let plan = try thumbnailPlan(input)
        guard case .groupRows(let plannedRows) = plan.arrangement else {
            throw TestFailure(description: "expected grouped Auto rows")
        }
        try expectEqual(plannedRows.count, 1)
        try expect(plannedRows[0].groups.allSatisfy { $0.itemRows.count == 1 },
                   "grouped auto-fit wrapped items inside a group")
    ...
    private static func testAutoPresentation() throws {
        let items = [
            ...
            PreviewLayoutItem(id: 5, aspectRatio: 1.0)
        ]
        let input = PreviewLayoutInput(
            content: .ungrouped(items),
            preferredImageHeight: 120,
            availablePanelSize: NSSize(width: 696, height: 600),
            backingScale: 2
        )
        let plan = try thumbnailPlan(input)
        ...
        try expect(plan.imageHeight < input.preferredImageHeight, "auto-fit fixture did not reduce its height")
    ...
    private static func testCompactChromeGeometry() throws {
        for size in PreviewContentSize.allCases {
            for showsClose in [false, true] {
                let container = NSView(frame: NSRect(x: 0, y: 0, width: 348, height: 50))
                container.translatesAutoresizingMaskIntoConstraints = false
                let icon = NSView()
                let title = NSTextField(labelWithString: "A document title long enough to need truncation")
                let status = NSTextField(labelWithString: "Desktop 10 · Minimized")
                let close = showsClose ? NSView() : nil
                title.font = .systemFont(ofSize: size.style.titleFontSize)
                title.setContentCompressionResistancePriority(.defaultLow, for: .horizontal)
                status.lineBreakMode = .byTruncatingTail
                let views = [icon, title, status] + (close.map { [$0] } ?? [])
                for view in views {
                    container.addSubview(view)
                    view.translatesAutoresizingMaskIntoConstraints = false
                }
                NSLayoutConstraint.activate(PreviewCardChromeLayout.compactConstraints(
                    container: container, appIconView: icon, titleLabel: title,
                    statusLabel: status, closeButton: close, contentStyle: size.style
                ) + [
                    container.widthAnchor.constraint(equalToConstant: 348),
                    container.heightAnchor.constraint(equalToConstant: 50)
                ])
                container.layoutSubtreeIfNeeded()
                try expect(title.frame.width > 80, "compact title became unreadable")
                try expect(icon.frame.maxX <= title.frame.minX, "icon overlapped title")
                try expect(title.frame.maxX <= status.frame.minX, "status overlapped title")
                if let close {
                    try expectNear(close.frame.width, 30)
                    try expect(status.frame.maxX <= close.frame.minX, "close button covered status")
                    try expect(container.bounds.contains(close.frame), "close button escaped list row")
                }
            }
        }
    }

    private static func testCompactScrollPosition() throws {
        let plan = PreviewCompactListPlan.make(
            itemCount: 50, rowHeight: 42, preferredWidth: 400,
            availablePanelSize: NSSize(width: 900, height: 420),
            panelPadding: 6, scrollerWidth: 15
        )
        let stack = verticalStack()
        let rows = (1...50).map { TestItemView(itemID: UInt32($0), size: NSSize(width: plan.rowWidth, height: plan.rowHeight)) }
        let scroll = try require(PreviewPresentationLayout.installLegacyRows(
            mode: .wrap, rowViews: rows, contentSize: plan.contentSize,
            viewportSize: plan.viewportSize, needsScroll: plan.needsScroll, spacing: 0, in: stack
        ), "compact overflow omitted scroll container")
        stack.frame = NSRect(origin: .zero, size: plan.viewportSize)
        stack.layoutSubtreeIfNeeded()
        PreviewPresentationLayout.restoreScrollPosition(NSPoint(x: 0, y: 420), in: stack)
        try expectNear(PreviewPresentationLayout.scrollPosition(in: stack)!.y, 420)
        PreviewPresentationLayout.restoreScrollPosition(NSPoint(x: 0, y: 100_000), in: stack)
        try expectNear(scroll.contentView.bounds.maxY, plan.contentSize.height)
        PreviewPresentationLayout.restoreScrollPosition(NSPoint(x: -10, y: -10), in: stack)
        try expectNear(scroll.contentView.bounds.minY, 0)
        try expectEqual(descendants(of: TestItemView.self, in: stack).count, 50)
    }

    private static func testHorizontalWheel() throws {
        let stack = verticalStack()
        let scroll = PreviewPresentationLayout.installHorizontalScrollContainer(
            rowView: testRow(ids: [1, 2, 3, 4], cardSize: NSSize(width: 200, height: 100)),
            contentSize: NSSize(width: 800, height: 100),
            viewportSize: NSSize(width: 300, height: 115), showsScroller: true, in: stack
        )
        stack.frame = NSRect(x: 0, y: 0, width: 300, height: 115)
        stack.layoutSubtreeIfNeeded()
        let cgEvent = try require(CGEvent(
            scrollWheelEvent2Source: nil, units: .line, wheelCount: 1,
            wheel1: -3, wheel2: 0, wheel3: 0
        ), "could not construct mouse wheel event")
        let event = try require(NSEvent(cgEvent: cgEvent), "could not bridge mouse wheel event")
        scroll.scrollWheel(with: event)
        try expect(scroll.contentView.bounds.minX > 0, "vertical mouse wheel did not move horizontal shelf")
        try expectNear(scroll.contentView.bounds.minY, 0)
    }
